...
class DataLoader(torch.utils.data.dataloader.DataLoader):
    def __init__(
        ...
        PML_dataset = dataset
        while True:
            if type(PML_dataset) is Subset:
                PML_dataset = PML_dataset.dataset
            elif type(PML_dataset) is ConcatDataset:
                PML_dataset = PML_dataset.datasets[0]
            else:
                break

        if type(PML_dataset) is list:
            num_workers = 0
        elif isinstance(PML_dataset, FlairDataset) and PML_dataset.is_in_memory():
            num_workers = 0
