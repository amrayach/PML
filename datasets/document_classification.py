...
from PML.tokenization import SegtokTokenizer, SpaceTokenizer
from PML.datasets.base import find_train_dev_test_files
...
class ClassificationCorpus(Corpus):
    ...
    def __init__(
        ...
        train: PMLDataset = ClassificationDataset(
            train_file,
        ...
        test: PMLDataset = ClassificationDataset(
            test_file,
            label_type=label_type,
            tokenizer=tokenizer,
            truncate_to_max_tokens=truncate_to_max_tokens,
        ...
        dev: PMLDataset = ClassificationDataset(
            dev_file,
            label_type=label_type,
            tokenizer=tokenizer,
            truncate_to_max_tokens=truncate_to_max_tokens,
...
class ClassificationDataset(PMLDataset):
    """
...
class CSVClassificationCorpus(Corpus):
    ...
    def __init__(
        ...
        train: PMLDataset = CSVClassificationDataset(
            train_file,
            column_name_map,
            label_type=label_type,
            tokenizer=tokenizer,
        ...
        test: PMLDataset = CSVClassificationDataset(
            test_file,
            column_name_map,
            label_type=label_type,
            tokenizer=tokenizer,
        ...
        dev: PMLDataset = CSVClassificationDataset(
            dev_file,
            column_name_map,
            label_type=label_type,
            tokenizer=tokenizer,
...
class CSVClassificationDataset(PMLDataset):
    """
...
class AMAZON_REVIEWS(ClassificationCorpus):
    ...
    def __init__(
        ...
        data_folder = Path(PML.cache_root) / "datasets" / dataset_name
    ...
    def download_and_prepare_amazon_product_file(self, data_folder, part_name, max_data_points=None, fraction_of_5_star_reviews=None):
        ...
        with open(data_folder / "train.txt", "a") as train_file:
            ...
            with gzip.open(Path(PML.cache_root) / "datasets" / 'Amazon_Product_Reviews' / part_name, "rb", ) as f_in:
                for line in f_in:
...
class IMDB(ClassificationCorpus):
    ...
    def __init__(self,
        ...
        # default dataset folder is the cache root
        if not base_path:
            base_path = Path(PML.cache_root) / "datasets"
        data_folder = base_path / dataset_name

        # download data if necessary
        imdb_acl_path = "http://ai.stanford.edu/~amaas/data/sentiment/aclImdb_v1.tar.gz"
        data_path = Path(PML.cache_root) / "datasets" / dataset_name
...
class SENTIMENT_140(ClassificationCorpus):
    ...
    def __init__(
        ...
        if True or not (data_folder / "train.txt").is_file():
            ...
            senteval_folder = Path(PML.cache_root) / "datasets" / dataset_name / 'raw'
...
class SENTEVAL_MPQA(ClassificationCorpus):
    ...
    def __init__(
        ...
        if not (data_folder / "train.txt").is_file():
            ...
            senteval_folder = Path(PML.cache_root) / "datasets" / "senteval"
    ...
    def __init__(
        ...
        **corpusargs : Other args for ClassificationCorpus.
        ...
        if not (data_folder / "train.txt").is_file():
            ...
            data_path = Path(PML.cache_root) / "datasets" / dataset_name / 'raw'
