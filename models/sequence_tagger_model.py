...
import PML.nn
from PML.data import Dictionary, Sentence, Label
from PML.datasets import SentenceDataset, DataLoader
from PML.embeddings import TokenEmbeddings, StackedEmbeddings, Embeddings
from PML.file_utils import cached_path, unzip_file
from PML.training_utils import Metric, Result, store_embeddings
...
def pad_tensors(tensor_list):
    ...
    template = torch.zeros(*shape, dtype=torch.long, device=PML.device)
...
class SequenceTagger(PML.nn.Model):
    def __init__(
            self,
            hidden_size: int,
        ...
        if loss_weights is not None:
            ...
            self.loss_weights = torch.FloatTensor(weight_list).to(PML.device)
        else:
        ...
        if word_dropout > 0.0:
            self.word_dropout = PML.nn.WordDropout(word_dropout)

        if locked_dropout > 0.0:
            self.locked_dropout = PML.nn.LockedDropout(locked_dropout)
    ...
    def _score_sentence(self, feats, tags, lens_):
        ...
        start = torch.tensor(
            [self.tag_dictionary.get_idx_for_item(START_TAG)], device=PML.device
        )
        start = start[None, :].repeat(tags.shape[0], 1)

        stop = torch.tensor(
            [self.tag_dictionary.get_idx_for_item(STOP_TAG)], device=PML.device
        )
        ...
        score = torch.FloatTensor(feats.shape[0]).to(PML.device)

        for i in range(feats.shape[0]):
            r = torch.LongTensor(range(lens_[i])).to(PML.device)
    ...
    def _calculate_loss(
        ...
        for s_id, sentence in enumerate(sentences):
            ...
            # add tags as tensor
            tag = torch.tensor(tag_idx, device=PML.device)
            tag_list.append(tag)
    ...
    def _fetch_model(model_name) -> str:
        ...
        if model_name == "de-historic-indirect":
            model_file = Path(PML.cache_root) / cache_dir / 'indirect' / 'final-model.pt'
            if not model_file.exists():
                cached_path('http://www.redewiedergabe.de/models/indirect.zip', cache_dir=cache_dir)
                unzip_file(Path(PML.cache_root) / cache_dir / 'indirect.zip', Path(PML.cache_root) / cache_dir)
            model_name = str(Path(PML.cache_root) / cache_dir / 'indirect' / 'final-model.pt')

        if model_name == "de-historic-direct":
            model_file = Path(PML.cache_root) / cache_dir / 'direct' / 'final-model.pt'
            if not model_file.exists():
                cached_path('http://www.redewiedergabe.de/models/direct.zip', cache_dir=cache_dir)
                unzip_file(Path(PML.cache_root) / cache_dir / 'direct.zip', Path(PML.cache_root) / cache_dir)
            model_name = str(Path(PML.cache_root) / cache_dir / 'direct' / 'final-model.pt')

        if model_name == "de-historic-reported":
            model_file = Path(PML.cache_root) / cache_dir / 'reported' / 'final-model.pt'
            if not model_file.exists():
                cached_path('http://www.redewiedergabe.de/models/reported.zip', cache_dir=cache_dir)
                unzip_file(Path(PML.cache_root) / cache_dir / 'reported.zip', Path(PML.cache_root) / cache_dir)
            model_name = str(Path(PML.cache_root) / cache_dir / 'reported' / 'final-model.pt')

        if model_name == "de-historic-free-indirect":
            model_file = Path(PML.cache_root) / cache_dir / 'freeIndirect' / 'final-model.pt'
            if not model_file.exists():
                cached_path('http://www.redewiedergabe.de/models/freeIndirect.zip', cache_dir=cache_dir)
                unzip_file(Path(PML.cache_root) / cache_dir / 'freeIndirect.zip', Path(PML.cache_root) / cache_dir)
            model_name = str(Path(PML.cache_root) / cache_dir / 'freeIndirect' / 'final-model.pt')
    ...
    def __str__(self):
        return super(PML.nn.Model, self).__str__().rstrip(')') + \
               f'  (beta): {self.beta}\n' + \
