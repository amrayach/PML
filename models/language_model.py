...
import PML
from PML.data import Dictionary
...
class LanguageModel(nn.Module):
    ...
    def get_representation(
        ...
        for chunk in chunks:
            ...
            t = torch.tensor(sequences_as_char_indices, dtype=torch.long).to(
                device=PML.device, non_blocking=True
            )
            batches.append(t)
    ...
    def load_language_model(cls, model_file: Union[Path, str]):
        ...
        state = torch.load(str(model_file), map_location=PML.device)
    ...
    def generate_text(
        ...
        with torch.no_grad():
            ...
            if len(prefix) > 1:
                ...
                input = torch.cat(char_tensors).to(PML.device)
