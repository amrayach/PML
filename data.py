...
from PML.file_utils import Tqdm
...
class DataPoint:
    """
    This is the parent class of all data points in PML (including Token, Sentence, Image, etc.). Each DataPoint
...
class Image(DataPoint):
    ...
    def get_embedding(self) -> torch.tensor:
        ...
        return torch.tensor([], device=PML.device)

    def set_embedding(self, name: str, vector: torch.tensor):
        device = PML.device
        if (PML.embedding_storage_mode == "cpu") and len(self._embeddings.keys()) > 0:
...
class Token(DataPoint):
    ...
    def get_each_embedding(self, embedding_names: Optional[List[str]] = None) -> torch.tensor:
        ...
        for embed in sorted(self._embeddings.keys()):
            ...
            embed = self._embeddings[embed].to(PML.device)
            if (PML.embedding_storage_mode == "cpu") and embed.device != PML.device:
                embed = embed.to(PML.device)
...
class Sentence(DataPoint):
    ...
    def __init__(
        ...
        if isinstance(use_tokenizer, Tokenizer):
            ...
            from PML.tokenization import TokenizerWrapper
...
class Corpus:
    def __init__(
        ...
        self._train: PMLDataset = train
        self._test: PMLDataset = test
        self._dev: PMLDataset = dev

    @property
    def train(self) -> PMLDataset:
        return self._train

    @property
    def dev(self) -> PMLDataset:
        return self._dev

    @property
    def test(self) -> PMLDataset:
        return self._test
    ...
    def _filter_long_sentences(dataset, max_charlength: int) -> Dataset:
        ...
        from PML.datasets import DataLoader
