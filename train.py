...
def evaluate(model, validation_generator, criterion, epoch, writer, log_file, print_every=25):
    ...
    writer.add_scalar('Test/f1-weighted/epoch', f1_test_weighted, epoch + iter)
    writer.add_scalar('Test/f1-micro/epoch', f1_test_micro, epoch + iter)
    writer.add_scalar('Test/f1-macro/epoch', f1_test_macro, epoch + iter)
...
def main():
    ...
    if args.getboolean('Model', 'visualize_model_graph'):
        x = torch.zeros((args.getint('Train', 'batch_size'),
                       ...
                       args.getint('DataSet', 'l0')))
        out = model(x)
        make_dot(out).render("CharacterLevelCNN", format="png")
